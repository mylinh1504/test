<?php
if ( ! class_exists( 'Legal_News_Author_Info_Widget' ) ) {
	/**
	 * Adds Legal_News_Author_Info_Widget Widget.
	 */
	class Legal_News_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$legal_news_author_info_widget_ops = array(
				'classname'   => 'ascendoor-widget author-info',
				'description' => __( 'Retrive Author Information Widget', 'legal-news' ),
			);
			parent::__construct(
				'legal_news_author_info_widget',
				__( 'Ascendoor Author Info Widget', 'legal-news' ),
				$legal_news_author_info_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			<?php } ?>
			<div class="legal-news-section-body">
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'legal-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'legal-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'legal-news' ); ?></label>:<br />
				...
				<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'legal-news' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'legal-news' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'legal-news' ); ?>" style="margin-top:5px;" /><br/>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'social_link_number' ) ); ?>"><?php esc_html_e( 'Number of links to show:', 'legal-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'legal-news' ); ?>:</label>
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link_new_tab' ), 'legal-news' ); ?>"  <?php checked( $open_link_new_tab, true ); ?> />
			</p>
				<?php
				for ( $i = 1; $i <= $social_link_number; $i++ ) {
					...
					?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php sprintf( esc_html__( 'Link %s :', 'legal-news' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link_new_tab']  = legal_news_sanitize_checkbox( $new_instance['open_link_new_tab'] );
 ...
 *
 * @since Legal News 1.0
 **/
function legal_news_author_info_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'author-info-widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.js', array( 'jquery' ), LEGAL_NEWS_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'legal_news_author_info_widget_image_upload_enqueue' );
